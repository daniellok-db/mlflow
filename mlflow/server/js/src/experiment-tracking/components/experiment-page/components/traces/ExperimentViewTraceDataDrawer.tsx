...
type ModelTraceRendererWindow = Window & {
  injectTrace: (trace: ModelTrace) => void;
};
  ...
  const renderContent = () => {
    ...
    if (combinedModelTrace) {
      return (
        ...
        >
          <iframe
            src="lib/ml-model-trace-renderer/index.html"
            title="Model trace explorer"
            css={{ border: 0, height: '100%' }}
            onLoad={({ target }: any) => {
              const targetWindow: ModelTraceRendererWindow = target?.contentWindow;
              if (targetWindow.injectTrace && combinedModelTrace) {
                targetWindow.injectTrace(combinedModelTrace);
              }
            }}
          ></iframe>
        </div>
